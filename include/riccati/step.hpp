...
std::tuple<std::complex<Scalar>, std::complex<Scalar>, Scalar, bool> nonosc_step(SolverInfo &&info, Scalar x0, Scalar h,
  ...
  auto cheby = std::tie(yprev, dyprev, xprev)
      = spectral_chebyshev(info, x0, h, y0, dy0, 0);
  ...
  while (maxerr > epsres) {
    ...
    cheby = spectral_chebyshev(info, x0, h, y0, dy0,
                               static_cast<int>(std::log2(N / info.nini)));
    auto &&y = std::get<0>(cheby) auto &&dy = std::get<1>(cheby);
...
template <typename SolverInfo, typename Scalar>
std::tuple<std::complex<Scalar>, std::complex<Scalar>, std::complex<Scalar>, bool, std::complex<Scalar>>  osc_step(SolverInfo &&info, Scalar x0, Scalar h, std::complex<Scalar> y0,
  ...
  auto delta = [&](const vectorc_t &r, const vectorc_t &y) {
    return -r / (2.0 * (y + gs));
  };
  auto R = [&](const vectorc_t &d) {
    return 2.0 / h * (Dn * d) + d.array().square();
  };
  vectorc_t Ry
      = complex_t(0.0, 1.0) * 2.0 * (1.0 / h * (Dn * ws) + gs.cwiseProduct(ws));
  complex_t maxerr = Ry.cwiseAbs().maxCoeff();
  ...
  vectorc_t f1;
  if (info.denseout) {
    f1 = (h / 2.0 * (info.intmat * du1)).array().exp();
  } else {
    f1 = (h / 2.0 * (info.quadwts.asDiagonal() * du1)).array().exp();
  }
  vectorc_t f2 = f1.conjugate();
  vectorc_t du2 = du1.conjugate();
  complex_t ap = (dy0 - y0 * du2(du2.size() - 1.0))
                 / (du1(du1.size() - 1.0) - du2(du2.size() - 1.0));
  complex_t am = (dy0 - y0 * du1(du1.size() - 1))
                 / (du2(du2.size() - 1.0) - du1(du1.size() - 1.0));
