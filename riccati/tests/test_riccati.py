...
def test_denseoutput_warn():
    ...
    with warnings.catch_warnings(record=True) as w:
        xs, ys, dys, ss, ps, stypes, yeval = solve(
            info, xi, xf, yi, dyi, xeval=xeval, eps=eps, epsh=epsh, warn=True
        )
...
def test_solve_airy():
    ...
    dyi = -sp.airy(-xi)[1] - 1j * sp.airy(-xi)[3]
    xs, ys, dys, ss, ps, stypes, yeval = solve(
        info, xi, xf, yi, dyi, eps=eps, epsh=epsh
    )
    xs = np.array(xs)
    ys = np.array(ys)
    ytrue = np.array([mpmath.airyai(-x) + 1j * mpmath.airybi(-x) for x in xs])
...
def test_solve_airy_backwards():
    ...
    dyi = -sp.airy(-xi)[1] - 1j * sp.airy(-xi)[3]
    xs, ys, dys, ss, ps, stypes, yeval = solve(
        info, xi, xf, yi, dyi, eps=eps, epsh=epsh, hard_stop=True
    )
    xs = np.array(xs)
    ys = np.array(ys)
    ytrue = np.array([mpmath.airyai(-x) + 1j * mpmath.airybi(-x) for x in xs])
...
def test_denseoutput_backwards_xfor():
    ...
    xeval = np.linspace(xi, xf, Neval)
    xs, ys, dys, ss, ps, stypes, yeval = solve(
        info, xi, xf, yi, dyi, xeval=xeval, eps=eps, epsh=epsh, hard_stop=True
    )
    ytrue = np.array([mpmath.airyai(-x) + 1j * mpmath.airybi(-x) for x in xeval])
    yerr = np.abs((ytrue - yeval) / ytrue)
    maxerr = max(yerr)
